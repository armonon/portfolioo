...
export const radarLanes: RadarLane[] = [
  {
    ...
    bullets: ["Sattari Hub internal alpha download is available", "Hub embeds 7 Sattari products / 18 AU-VST3-Standalone payload entries", "Auto Pitch performer-modes build remains internal alpha and needs real vocal/DAW listening tests"],
...
export const radarSoftwareProjects: RadarSoftwareProject[] = [
  ...
  },
  {
    ...
    image: "/photos/auto-pitch-example.svg",
    summary: "Self-contained Sattari Hub installer/updater alpha plus the older suite test pack for Apple Silicon Mac validation.",
    description: "Sattari Hub is the new downloadable internal alpha app for installing, updating, and hosting the current Sattari Audio software from one place. The packaged archive embeds the available AU/VST3/Standalone payloads so another Apple Silicon Mac can test without cloning the repo. It is explicitly not notarized, not a beta, not a public release, not DAW-validated, and not sale-ready. Testing should focus on installation/update flow, Gatekeeper friction, DAW rescan behavior, and real listening notes for Auto Pitch/VoxKey/StemDeck and the other plugin builds.",
    ...
    testingFocus: "Installer/updater validation only: download Hub on an Apple Silicon Mac, install/update the embedded internal-alpha builds, rescan the DAW, and record blockers without treating the suite as beta or sale-ready.",
  ...
  },
  {
    ...
    description: "Sattari StemDeck v0.3.6 laptop macOS 13+ pre-alpha b9f4909 is available as a truth-labeled internal test package for DAW/listening tests. The build includes VST3 and Standalone artifacts compiled for arm64 macOS 13+ with no Homebrew audio dylib dependency, plus external Full/Drums/Bass/Music/Vocals stem lanes, optional Demucs setup scripts, host BPM sync, Beat Sync with per-deck Source BPM and Beat Grid Offset, LiveKey analysis, target-key matching/shifting, Preserve/key-lock routing, pads, mic monitoring, WAV recording, and the latest separator-path hardening. It is not sale-ready until known-BPM/key DAW tests prove sync, LiveKey accuracy, and audio quality.",
    tags: ["Audio", "Plugin", "Remix", "Internal pre-alpha"],
    downloads: [
      { label: "Download StemDeck v0.3.6 laptop macOS 13+ b9f4909", href: "/downloads/stemdeck-plugin-alpha-v0.3.6-laptop-macos13-b9f4909.tar.gz" },
    ],
    testingFocus: "Install the laptop macOS 13+ VST3/Standalone pre-alpha, optionally run the included Demucs setup, load known-BPM/key tracks or prepared stems, then validate beat sync, LiveKey accuracy, Preserve/key-shift quality, recording, and DAW save/reopen behavior without treating it as sale-ready.",
    testSteps: ["Download the v0.3.6 laptop macOS 13+ b9f4909 package", "Run scripts/install_user_plugins.sh or copy the VST3/Standalone artifact manually", "Load two known-BPM/key loops or songs in a DAW", "Set each deck Source BPM, enable Host Sync + Beat Sync, and nudge Beat Grid Offset", "Enable Preserve + LiveKey, set a target key, and listen for matching quality/artifacts", "Record a 60-second WAV and document drift, clicks, false key readings, or crashes"],
  },
  {
    id: "auto-pitch",
    title: "Auto Pitch",
...
export const radarEvidenceLedger: RadarEvidenceItem[] = [
  ...
  },
  {
    ...
    latestProof: "StemDeck v0.3.6 laptop macOS 13+ pre-alpha b9f4909 package is downloadable with VST3/Standalone artifacts and optional Demucs setup scripts; validation passed build, codesign, macOS minos 13.0 check, no Homebrew audio dylib dependency check, granular/beat-sync/live-key/architecture/separator-command contract checks, and pluginval strictness 5.",
