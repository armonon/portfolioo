...
export const radarLanes: RadarLane[] = [
  {
    ...
    bullets: ["Sattari Hub 0.1.2 internal alpha download is available", "StemDeck 0.4.0 is now packaged as its own standalone Mac DJ app alpha, separate from the plugin package", "Auto Pitch 0.1.4 real correction fix is available through the Hub update feed; real DAW/listening validation is still required"],
    nextAction: "Download either Sattari Hub 0.1.2 or the standalone StemDeck 0.4.0 app alpha from Product Radar on an Apple Silicon Mac, open/install the app, then record install/listening/UX notes.",
    ...
    detail: {
      ...
      downloads: [
        ...
        { label: "Download StemDeck 0.4.2 standalone Mac app alpha", href: "/downloads/stemdeck-app-alpha-v0.4.2-standalone-bundled-rubberband.tar.gz" },
...
export const radarSoftwareProjects: RadarSoftwareProject[] = [
  ...
  },
  {
    ...
    image: "/photos/auto-pitch-example.svg",
    summary: "Standalone Sattari StemDeck Mac DJ app alpha with top live mixer/stem decks and bottom arrangement recorder workspace.",
    description: "Sattari StemDeck v0.4.2 standalone-app alpha is available as its own Apple Silicon Mac application package. The app-first package contains Sattari StemDeck.app at the top level, an install_app.sh helper for ~/Applications, the split live-mixer / arrangement-recorder UI, optional Demucs setup scripts, bundled Rubber Band/libsamplerate runtime dylibs, Rubber Band-backed time/pitch validation output, and architecture contracts. It is not notarized or sale-ready; the bottom arrangement space is an alpha workflow/capture surface, not a complete editable Ableton-style timeline yet.",
...
export const radarEvidenceLedger: RadarEvidenceItem[] = [
  ...
  },
  {
    ...
    latestProof: "StemDeck v0.4.2 standalone Mac app alpha is packaged/downloadable with Sattari StemDeck.app at the top level; validation passed standalone build, bundled dylib/no-/opt-homebrew runtime check, codesign strict verify, granular render contract, Rubber Band time/pitch benchmark, and architecture contract.",
