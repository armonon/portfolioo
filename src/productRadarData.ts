...
export const radarLanes: RadarLane[] = [
  {
    ...
    bullets: ["Auto Pitch is the lead product lane", "Sale-ready standard over demo hype", "StemDeck v0.3.5 alpha download is available for DAW testing"],
    nextAction: "Run DAW/listening validation on the StemDeck v0.3.5 alpha package with known BPM/key tracks.",
    blocker: "StemDeck has an installable internal-alpha package, but needs DAW/listening validation before sale-ready language.",
    ...
    detail: {
      ...
      downloads: [
        ...
        { label: "Download StemDeck v0.3.5 alpha", href: "/downloads/stemdeck-plugin-alpha-v0.3.5-live-key.tar.gz" },
        { label: "Open Sattari site", href: "https://sattari.netlify.app" },
      ],
      highlights: ["Audio plugin/product lane", "Preview pack available", "StemDeck alpha download available; DAW/listening validation still required"],
    },
  },
  {
    id: "creator-tools",
...
export const radarSoftwareProjects: RadarSoftwareProject[] = [
  ...
  },
  {
    ...
    status: "Internal alpha download",
    image: "/photos/auto-pitch-example.svg",
    summary: "Four-deck Sattari Audio remix plugin/app alpha with external stem lanes, beat sync, live key analysis, target-key matching, pads, mic monitor, and recording.",
    description: "Sattari StemDeck v0.3.5 is available as a truth-labeled internal alpha package for DAW/listening tests. The build includes VST3 and Standalone artifacts, external Full/Drums/Bass/Music/Vocals stem lanes, host BPM sync, Beat Sync with per-deck Source BPM and Beat Grid Offset, LiveKey analysis, target-key matching/shifting, Preserve/key-lock routing, pads, mic monitoring, and WAV recording. It is not sale-ready until known-BPM/key DAW tests prove sync, LiveKey accuracy, and audio quality.",
    tags: ["Audio", "Plugin", "Remix", "Internal alpha"],
    downloads: [
      { label: "Download StemDeck v0.3.5 alpha", href: "/downloads/stemdeck-plugin-alpha-v0.3.5-live-key.tar.gz" },
    ],
    testingFocus: "Install the VST3/Standalone alpha, load known-BPM/key tracks or prepared stems, then validate beat sync, LiveKey accuracy, Preserve/key-shift quality, recording, and DAW save/reopen behavior without treating it as sale-ready.",
    testSteps: ["Download the v0.3.5 alpha package", "Run scripts/install_user_plugins.sh or copy the VST3/Standalone artifact manually", "Load two known-BPM/key loops or songs in a DAW", "Set each deck Source BPM, enable Host Sync + Beat Sync, and nudge Beat Grid Offset", "Enable Preserve + LiveKey, set a target key, and listen for matching quality/artifacts", "Record a 60-second WAV and document drift, clicks, false key readings, or crashes"],
  },
  {
    id: "auto-pitch",
    title: "Auto Pitch",
...
export const radarEvidenceLedger: RadarEvidenceItem[] = [
  ...
  },
  {
    ...
    latestProof: "StemDeck v0.3.5 internal alpha package is downloadable with VST3/Standalone artifacts; validation passed build, codesign, granular/beat-sync/live-key contract checks, and pluginval strictness 5.",
    blocker: "Still needs real DAW/listening validation with known BPM/key material; no AI stem separation, no sale-grade time-stretch/key detector, and hosted-plugin routing remains scaffold-only.",
    nextStep: "Install the alpha package from Project Radar and run the DAW/listening matrix for sync drift, LiveKey accuracy, Preserve/key-shift artifacts, recording, and save/reopen behavior.",
